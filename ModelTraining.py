...
# duplicates_in_split = len(overlapping)
# print(f"❗ Overlapping samples between train/test: {duplicates_in_split}")
#
# # === 8. Scale data (for SVM and LR only) ===
...
#     # print(f"ROC AUC:   {roc:.4f}" if roc else "ROC AUC: Not available")
#
...
# # cross_validate_model("Logistic Regression", lr, X, y, scale=True)
# # === 12. Identify Best Model ===
# best_by_f1 = max(model_scores.items(), key=lambda x: x[1]['f1_score'])
# # best_by_roc = max(
# #     ((k, v) for k, v in model_scores.items() if v['roc_auc'] is not None),
...
#
# # print("\n🏆 Best Model by ROC AUC:", best_by_roc[0])
# # print("ROC AUC:", best_by_roc[1]['roc_auc'])
#
# # Plot again for best model
# best_model_name = best_by_f1[0]
# best_model = best_by_f1[1]['model']
# X_plot = X_test_scaled if best_by_f1[1]['scaled'] else X_test
# y_pred = best_model.predict(X_plot)
# y_proba = best_model.predict_proba(X_plot)[:, 1]
#
# # === 13. Bar Chart Comparison of All Models ===
...
# # joblib.dump(lr, r"logistic_regression_model.pkl")
